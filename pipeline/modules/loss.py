...
class Loss:
    ...
    def forward(self, box_pseudo_gt: dict, box3d_branch: torch.Tensor, smoke=None):
        ...
        with torch.no_grad():
            ...
            if box3d_branch_metric is not None:
                box3d_branch_metric_filtered = self._filter_with_3d_radius(box3d_branch_metric,
                                                                           box_3d_gt=copy.deepcopy(box_pseudo_gt["3d"]),
                                                                           radius=2)
        ...
        if "score" == self._type:
            ...
            res = self._get_3d_weighted_score(box3d_branch, box_3d_gt=copy.deepcopy(box_pseudo_gt["3d"]))
        elif "3d_weighted_y" == self._type:
            res = self._get_3d_weighted_y(box3d_branch, box_3d_gt=copy.deepcopy(box_pseudo_gt["3d"]))
        return res
    ...
    def _get_3d_weighted_score(cls, box3d_branch: torch.Tensor, box_3d_gt):
        ...
        sigma = torch.tensor([2 / 2, 2 / 2, 2 / 2, 2 / 2, 20 / 2, 2 / 2], device=box3d_branch.device)
        # input x
        x = box3d_branch[:, 6:12]
    ...
    @classmethod
    def _get_3d_weighted_y(cls, box3d_branch: torch.Tensor, box_3d_gt):
        device = box3d_branch.device
        ...
        y_pred = box3d_branch[:, 10]
        y_gt = box_3d_gt_location[1]
        y_score = torch.flatten(torch.square(torch.sub(y_pred, y_gt)))

        loss = -1 * torch.sum(prob_obj_softmax * y_pred)
        return loss
